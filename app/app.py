...
cost = st.sidebar.number_input(
    "Total cost of ride", 0.0, 100000.0, 100.0, 0.01
)
...
if run_button:
    ...
    st.write(
        f"For a trip of {cost} dollars starting at \
            {locations[0]['display_name']}, the optimal split is as follows:"
    )
    for i in range(num_locations - 1):
        st.write(
            f"Player {i + 1} pays {round(shap_vals[i],2)} dollars\
                 to travel to {locations[i + 1]['display_name']}"
        )
